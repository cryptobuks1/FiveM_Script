...
AddEventHandler(
    "esx_inventoryhud:openVaultInventory",
    function(data)
...
function refreshVaultInventory()
    data = exports['zofirsty_vault']:getZoFisrtyrVaultLicense()
    ESX.TriggerServerCallback(
        "zofirsty_vault:getVaultInventory",
        function(inventory)
    ...
    function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            ...
            if data.item.type == "item_weapon" then
				...
				TriggerServerEvent("zofirsty_vault:putItem", --[[ESX.GetPlayerData().identifier,--]] vaultType, data.item.type, data.item.name, count)
            else
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            ...
            TriggerServerEvent("zofirsty_vault:getItem", --[[ESX.GetPlayerData().identifier,--]] vaultType, data.item.type, data.item.name, count)
        end
