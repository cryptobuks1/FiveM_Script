	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions',
	{
			...
			'default', GetCurrentResourceName(), 'citizen_interaction',
			{
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'unhandcuff' then
						...
						playerlocation = GetEntityForwardVector(PlayerPedId())
						playerCoords = GetEntityCoords(GetPlayerPed(-1))
						TriggerServerEvent('esx_policejob:unhandcuff', GetPlayerServerId(closestPlayer), playerheading, playerCoords, playerlocation)
		...
		elseif data.current.value == 'vehicle_interaction' then
			...
			if DoesEntityExist(vehicle) then
				...
				--table.insert(elements, {label = "ยึดรถ",		value = 'impound_police'})
			end
			...
			'default', GetCurrentResourceName(), 'vehicle_interaction',
			{
				...
				elseif DoesEntityExist(vehicle) then
					...
					elseif action == 'impound_police' then
...
function OpenBodySearchMenu(player)
	...
	ESX.TriggerServerCallback('esx_policejob:getOtherPlayerData', function(data)
		...
		TriggerEvent("esx_inventoryhud:openPlayerInventory", GetPlayerServerId(player), data.firstname, data.lastname, data.weapons, data1)
...
function CreateBillingPrice(player, value)
	...
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'dialog_createbill_price',
	{
		...
		if tonumber(data2.value) then
			TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_police',"Fine: "..value, tonumber(data2.value))
			menu2.close()
			TriggerEvent("pNotify:SendNotification", {
...
function SearchHistoryVehicleByName()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	{
		...
		if data.value == nil then
			...
			ESX.TriggerServerCallback('esx_policejob:getVehicleByName', function(result, found)
				if found then
...
function SearchHistoryHouseByName()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	{
		...
		if data.value == nil then
			...
			ESX.TriggerServerCallback('esx_policejob:getHouseByName', function(result, found)
				if found then
...
function ImpoundVehiclePolice(vehicle)
	TriggerServerEvent('esx_policejob:updateVehicleFromPolice', GetVehicleNumberPlateText(vehicle))
	ESX.Game.DeleteVehicle(vehicle) 
	TriggerEvent("pNotify:SendNotification", {
		text = '<strong class="green-text">พาวน์รถเรียบร้อยแล้ว</strong>',
		type = "success",
