...
function OpenArmoryMenu(station)
	local elements = {
		...
		{label = _U('storage'), value = 'storage'}
	}
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		...
		elements = {
			...
			{label = _U('tablet'), value = 'tablet'},
			{label = _U('radar'), value = 'radar'}
	}}, function(data, menu)
		if data.current.value == 'citizen_interaction' then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif data2.current.value == 'jail' then
		...
		TriggerEvent( "wk:openRemote" )
			...
			ESX.ShowNotification(_U('not_sit_in_vehicle'))
			end
		...
		elseif data.current.value == 'object_spawner' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				ESX.Game.SpawnObject(data2.current.model, objectCoords, function(obj)
					TaskStartScenarioInPlace(playerPed, 'CODE_HUMAN_MEDIC_TEND_TO_DEAD', 0, true)
					exports['progressBars']:startUI(7500, _U('object_placing'))
					Citizen.Wait(5000)
