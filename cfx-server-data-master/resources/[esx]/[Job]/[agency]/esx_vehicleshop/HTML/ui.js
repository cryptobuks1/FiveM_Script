    ...
    function LoadCarsPage()
    {
        ...
        mpage = Math.ceil(result.length / 6)
            for (let n_page = 0; n_page < Math.ceil(result.length / 6); n_page++) {
                ...
                while (index < result.length && count < 8) {
                    ...
                    html += `<div class="col-3 mb-4">
                                <div class="card h-100">
                                    ...
                                    <div class="card-body">
                                        <h5 class="card-title text-white">`+car.name+`</h5>
                                        <p class="card-text text-white">ยี่ห้อ : <b>`+car.categoryLabel+`</b></p>
                                        <p class="card-text text-white">ราคา : <b>`+car.price+` $</b></p>
                                    </div>
                                    <div class="card-footer border-0 ">
                                        <button type="button" id="action1" data-value="buy" data-model="`+ car.model +`" class="btn btn-danger w-auto buy">ซื้อสินค้า</button>
                                        <button type="button" id="action2" data-value="test-drive" data-model="`+ car.model +`" class="btn btn-success w-auto float-right btn-medium test-drive">ทดลองขับ</button>
                                    </div>
                                </div>
                            </div>`
                    index++;
