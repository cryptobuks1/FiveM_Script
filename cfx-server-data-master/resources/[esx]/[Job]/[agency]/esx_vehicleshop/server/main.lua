...
ESX.RegisterServerCallback('esx_vehicleshop:buyVehicle', function (source, cb, vehicleModel)
	...
	if xPlayer.getMoney() >= vehicleData.price then
		...
		TriggerClientEvent('discordbot:buycar_cl', source, vehicleData.name, vehicleData.price, xPlayer.name)
		cb(true)
		...
		if result[1] then -- does the owner match?
			...
			if vehicle.model == model then
				if vehicle.plate == plate then
					...
					print(('esx_vehicleshop: %s Matrícula inválida!'):format(xPlayer.identifier))
					cb(false)
				end
			else
				print(('esx_vehicleshop: %s Modelo inválido!'):format(xPlayer.identifier))
				cb(false)
			end
		end
