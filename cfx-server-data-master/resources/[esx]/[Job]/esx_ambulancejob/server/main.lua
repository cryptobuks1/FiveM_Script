...
RegisterServerEvent('esx_ambulancejob:animRevive')
AddEventHandler('esx_ambulancejob:animRevive', function(target, Position, Rotation)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.job.name == 'ambulance' then
		TriggerClientEvent('esx_ambulancejob:animTarget', target, Position, Rotation)
		TriggerClientEvent('esx_ambulancejob:animDocktor', source, Position, Rotation)
	else
...
ESX.RegisterUsableItem('medikit', function(source)
	if not playersHealing[source] then
		...
		local sendToDiscord = '**'	.. xPlayer.name .. '** ใช้ **' .. ESX.GetItemLabel('medikit') .. '** จำนวน **1**'
		TriggerEvent('azael_discordlogs:sendToDiscord', 'DocUseItem', sendToDiscord, xPlayer.source, '^2')
...
ESX.RegisterUsableItem('bandage', function(source)
	if not playersHealing[source] then
		...
		local sendToDiscord = '**'	.. xPlayer.name .. '** ใช้ **' .. ESX.GetItemLabel('bandage') .. '** จำนวน **1**'
		TriggerEvent('azael_discordlogs:sendToDiscord', 'DocUseItem', sendToDiscord, xPlayer.source, '^2')
...
function CheckAmbulance()
	if delay < GetGameTimer() then
		if ESX.GetJobCount then
			local temp = ESX.GetJobCount("ambulance")
			ambulance = temp
		else
			local temp = 0
...
ESX.RegisterServerCallback('esx_ambulancejob:getMedicOnline', function(source, cb)
	cb(CheckAmbulance())
end)
