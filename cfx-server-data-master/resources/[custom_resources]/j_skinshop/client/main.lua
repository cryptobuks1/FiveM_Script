...
function OpenShopMenu()
	...
	function(data, menu)
		if data.current.value == 'outfit' then
						...
						if data.current.value == 'change_cloth' then
							...
							ESX.TriggerServerCallback('esx_property:getPlayerDressing', function(dressing)
								...
								function(data2, menu)
									...
									ESX.TriggerServerCallback('j_skinshop:buyClothes', function(bought)
										if bought then
			...
			TriggerEvent("j_skinshop:toggleMenu", "clotheshop")
		end
		currentAction     = 'shop_menu'
		--inMenu = false
		...
		if data.current.value == 'yes' then
			ESX.TriggerServerCallback('j_skinshop:buyClothes', function(bought)
				if bought then
					...
					ESX.TriggerServerCallback('j_skinshop:checkPropertyDataStore', function(foundStore)
						if foundStore then
							ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'save_dressing', {
								...
								if data2.current.value == 'yes' then
									ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'outfit_name', {
										...
										TriggerEvent('skinchanger:getSkin', function(skin)
											TriggerServerEvent('j_skinshop:saveOutfit', data3.value, skin)
...
RegisterNUICallback("addItemtoInventory", function(data)
	for k, v in pairs(data) do
		TriggerServerEvent('j_skinshop:buyItem', "skin_"..k.."_"..v, 1)
	  end
end)

AddEventHandler('j_skinshop:hasEnteredMarker', function(zone)
	currentAction     = 'shop_menu'
	currentActionMsg  = _U('press_menu')
	currentActionData = {}
end)

AddEventHandler('j_skinshop:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	currentAction = nil
	inMenu = false
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not hasAlreadyEnteredMarker) or (isInMarker and lastZone ~= currentZone) then
			...
			TriggerEvent('j_skinshop:hasEnteredMarker', currentZone)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('j_skinshop:hasExitedMarker', lastZone)
		end
