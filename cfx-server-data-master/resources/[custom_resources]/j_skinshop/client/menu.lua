...
RegisterNetEvent("j_skinshop:toggleMenu")
AddEventHandler("j_skinshop:toggleMenu", function(restriction)
...
RegisterNUICallback("exit", function(data)
	...
	if data == "clothes" then 
		TriggerEvent("j_skinshop:hasExitedMarker")
	else
...
RegisterNUICallback("update", function(data, cb)
	dados = tonumber(json.encode(data[1])) --หมวด
	tipo = tonumber(json.encode(data[2])) --ตำแหน่งไอเท็ม
	cor = 0
...
function setRoupa(dados, tipo, cor)
	...
	TriggerEvent('skinchanger:change', key, tipo) -- ชื่อหมวด , key
