    ...
    while true do
        ...
        if IsPedInAnyVehicle(player, false) and vehicleIsOn then
            ...
            if vehicleSpeed > Config.vehicle.maxSpeed then
                vehicleNailSpeed = math.ceil(280 -
                                                 math.ceil(
                        math.ceil(Config.vehicle.maxSpeed * 205) / Config.vehicle.maxSpeed))
            else
    ...
    while true do
        ...
        if IsPedInAnyVehicle(player, false) and GetIsVehicleEngineRunning(vehicle) then
            if IsControlJustReleased(0, Keys[Config.vehicle.keys.seatbelt]) and
				...
				local nameAnim = 'anim@mp_yacht@shower@male@'
				local anim = 'male_shower_enter_into_idle'
                if seatbeltIsOn then
                    ESX.Streaming.RequestAnimDict(nameAnim, function()
                        exports['progressBars']:startUI(3500, "กำลังคาดเข็มขัด")
                        TaskPlayAnim(player, nameAnim, anim, 8.0, -8, 3500, 49, 0, 0, 0, 0)
						SetNuiFocus(true, false)
                        TriggerEvent("pNotify:SendNotification", {
                            text = '<center><strong class="red-text">Seat belt closed.</strong><center>',
                            type = "success",
                            timeout = (3000),
                            layout = "centerLeft",
                            queue = "global"
                    ...
                    ESX.Streaming.RequestAnimDict(nameAnim, function()
                        exports['progressBars']:startUI(3500, "กำลังปลอดเข็มขัด...")
                        TaskPlayAnim(player, nameAnim, anim, 8.0, -8, 3500, 49, 0, 0, 0, 0)
						SetNuiFocus(true, false)
                        TriggerEvent("pNotify:SendNotification", {
                            text = '<center><strong class="green-text">Seat belt open.</strong><center>',
                            type = "error",
                            timeout = (3000),
                            layout = "centerLeft",
                            queue = "global"
        ...
        if IsControlJustPressed(1, Keys[Config.vehicle.keys.signalLeft]) and
            (has_value(vehiclesCars, vehicleClass) == true) then
        ...
        if IsControlJustPressed(1, Keys[Config.vehicle.keys.signalRight]) and
            (has_value(vehiclesCars, vehicleClass) == true) then
            if vehicleSignalIndicator == 'off' then
        ...
        if IsControlJustPressed(1, Keys[Config.vehicle.keys.signalBoth]) and
            (has_value(vehiclesCars, vehicleClass) == true) then
            if vehicleSignalIndicator == 'off' then
