...
C_VCFG = function(j, k, l, m)
    ...
    self.weight_max = nil
    self.CFGList = {
        {CFG_Type = "Vehicle_Weight_Class", DATA = {self.class_name}},
        {CFG_Type = "Vehicle_Weight_NameAndModel", DATA = {self.displayname, self.model}},
        {CFG_Type = "Vehicle_Weight_Plate", DATA = {self.plate}}
    }
    self.DATA = {["slots"] = nil, ["weight"] = nil, ["item_blacklist"] = nil, ["item_limit"] = nil, ["job"] = nil}
    self.GetDataInit = function()
        for n, o in ipairs(self.CFGList) do
            ...
            if p and o.DATA and #o.DATA > 0 then
                ...
                local s = p[q]
                if s then
                    for n, t in ipairs({"slots", "weight", "item_blacklist", "item_limit", "job"}) do
                        local u = s[t]
                        if u ~= nil then
                            self.DATA[t] = u
                        end
...
funcs.DATA_VEH_Vaild = function(G)
    if G then
        local H = {"plate", "slots_max", "weight_max", "class_name", "displayname", "model", "VHASH"}
...
funcs.getNumFormat = function(K)
    return tonumber(string.format("%.2f", K))
end
