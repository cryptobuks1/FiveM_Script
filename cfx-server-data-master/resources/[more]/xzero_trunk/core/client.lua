...
EventsInit = function()
    ...
    RegisEvent(
        false,
        events.cl["OnInventoryDataReloaded"],
        function(e, f, g)
            if not f then
                if e ~= nil then
                    e(false)
            ...
            TSC(
                ...
                function(h)
                    ...
                    local p = {
                        ...
                        limit = n,
                        max = o,
                        plate = CT["vehicle_textinfo_plate"]:format(
                            f.plate,
                            j,
                            f.slots_max > 0 and "/" .. f.slots_max or ""
                        ),
                        text = CT["vehicle_textinfo_text"]:format(
                            f.plate,
                            f.weight_max > 0 and n .. "/" .. o or n,
                            f.slots_max > 0 and j .. "/" .. f.slots_max or j
                        )
                    }
                    _InventoryHudInsObj.Inventory_Setup_Second({itemList = i, setInfoText = p})
    ...
    RegisEvent(
        ...
        function(q)
            ...
            if q and q.item and type(q.number) == "number" then
                ...
                if
                    r.type == "item_weapon" or
                        (r.type == "item_standard" or r.type == "item_account" or r.type == "item_money") and
                            r.count >= q.number
                 then
                    if not _C_Veh_Last.VCFG.Check_Item_BlackList(r.name) then
                        NotifyT("item_bl_error", nil, "error")
                        _Prograss = false
                        return
                    end
                    ...
                    TriggerServerEvent(events.sv["OnPut"], _DATA_VEH_Last, {item = q.item, count = q.number})
                    if Config.Anim then
    ...
    RegisEvent(
        ...
        function(q)
            ...
            if q and q.item and type(q.number) == "number" then
                ...
                TriggerServerEvent(events.sv["OnTake"], _DATA_VEH_Last, {item = q.item, count = q.number})
                if Config.Anim then
    ...
    if not IsEntityDead(t) then
        ...
        if _C_Veh_Last and xZero.Funcs.Vehicle_Vaild(_C_Veh_Last) then
            ...
            _DATA_VEH_Last = {
                ...
                VHASH = ("%s-%s-%s-%s"):format(
                    _C_Veh_Last.plate,
                    _C_Veh_Last.class_name,
                    _C_Veh_Last.displayname,
                    _C_Veh_Last.model
                )
            }
            TriggerEvent(
...
xZero.Funcs.VehicleFind = function()
    ...
    if IsPedInAnyVehicle(t, false) then
        ...
        if not Config.Vehicle_IsIn then
            NotifyT("vehicle_isin_error", {y.plate}, "error")
            return nil
        end
    else
...
xZero.Funcs.Vehicle_Vaild = function(J)
    local t = PlayerPedId()
    if Config.Vehicle_NPC_Disabled and J.IsNPC then
        NotifyT("vehicle_npc_disabled_error", {J.plate}, "error")
        return false
    end
    if Config.Vehicle_IsIn_DriverOnly then
        local K = J.Get_PedInSeat(-1)
        if K and K ~= t then
            NotifyT("vehicle_isin_driveronly_error", {J.plate}, "error")
            return false
        end
    end
    ...
    if J.Get_IsLocked() and not M then
        NotifyT("vehicle_locked_error", {J.plate}, "error")
        return false
    end
    local N = #(GetEntityCoords(t) - J.Get_Coords())
    if N > Config.Vehicle_Distance then
        NotifyT("vehicle_distance_error", {J.plate}, "error")
        return false
    end
    local O = false
    ...
    if O then
        NotifyT("vehicle_isin_pedany_error", {J.plate}, "error")
        return false
    end
    local S = {
        ["Vehicle_BlackList_Class"] = {J.class_name},
        ["Vehicle_BlackList_NameAndModel"] = {J.displayname, tostring(J.model)},
        ["Vehicle_BlackList_Plate"] = {J.plate}
    }
    for T, U in pairs(S) do
        if xZero.Funcs.Vehicle_BL(T, U) then
            NotifyT("vehicle_blacklist", {J.plate}, "error")
            return false
        end
    end
    local u = J.Get_VCFG()
    if u.slots_max == 0 then
        NotifyT("vehicle_slots_error", {J.plate}, "error")
        return false
    end
    if u.weight_max == 0 then
        NotifyT("vehicle_weight_error", {J.plate}, "error")
        return false
    end
    local V = ESX.GetPlayerData().job
    if not u.Check_Job(V.name) then
        NotifyT("vehicle_job_error", {J.plate}, "error")
        return false
    end
    return true
end
...
xZero.Funcs.Vehicle_Vaild_OT = function()
    local t = PlayerPedId()
...
xZero.C.Veh = function(x)
    ...
    self.IsVaild = function()
        if
            self.vehicle ~= nil and self.vehicle > 0 and DoesEntityExist(self.vehicle) and self.Get_Plate() and
                self.Get_DisplayName() and
                self.Get_Class()
         then
            if string.len(self.plate) == 8 then
                self.IsNPC = true
            end
            self.Vaild = true
            return self.Vaild
...
Notify = function(ab, type, af)
    ...
    TriggerEvent(
        ...
        {text = ab, type = type, queue = script_name, timeout = af, layout = layout}
    )
end
...
InventoryHudInsObjInit = function()
    ...
    while _InventoryHudInsObj == nil and GetGameTimer() - ax <= 10 * 1000 do
        TriggerEvent(
            _InventoryHudInsObjScriptName .. ":CL:GetInstanceObject",
            function(ay)
    ...
    RegisEvent(
        false,
        events.cl["OnInventoryHudInsObjReceive"],
        function(ay)
            if ay ~= nil then
                _InventoryHudInsObj = ay
            end
...
getItemLabel = {["item_account"] = function(az)
        return CT[az] or nil
    end, ["item_standard"] = function(az)
        return _Cache_Inventory[az].label
    end, ["item_weapon"] = function(az)
