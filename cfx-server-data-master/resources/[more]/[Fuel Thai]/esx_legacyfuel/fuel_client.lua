...
local currentCost = 10.0
local currentCash = 1000
local fuelSynced = false
local inBlacklisted = false

local Hookers = {
	{id = 1, VoiceName = "HOOKER_LEAVES_ANGRY", modelHash = "cs_chrisformage", x = 128.65, y = -1055.36, z = 29.19, heading = 156.4}, 
}

function ManageFuelUsage(vehicle)
	if not DecorExistOn(vehicle, Config.FuelDecor) then
		SetFuel(vehicle, math.random(70, 90))
...
AddEventHandler('fuel:startFuelUpTick2', function(pumpObject, ped, vehicle)
	currentFuel = GetVehicleFuelLevel(vehicle)
...
AddEventHandler('fuel:refuelInCar', function(pumpObject, ped, vehicle)
	...
	if (not ModelSpawned) then
		for i=1, #Hookers do
			...
			SpawnedPed = CreatePed(2, Hookers[i].modelHash, stringCoords.x + 0.5, stringCoords.y + 2.5, stringCoords.z, Hookers[i].heading, true, true)
			ModelSpawned = true
			TaskPedSlideToCoord(SpawnedPed, stringCoords.x, stringCoords.y + 0.5, stringCoords.z, 0, 500)
			Citizen.Wait(3000)
			TaskPedSlideToCoord(SpawnedPed, vehicleCoords.x, vehicleCoords.y, vehicleCoords.z, 0, 500)
			Citizen.Wait(2000)
			TaskTurnPedToFaceEntity(SpawnedPed, vehicle, 1000)
			Citizen.Wait(1000)
			SetCurrentPedWeapon(SpawnedPed, -1569615261, true)
			LoadAnimDict("timetable@gardener@filling_can")
			TaskPlayAnim(SpawnedPed, "timetable@gardener@filling_can", "gar_ig_5_filling_can", 2.0, 8.0, -1, 50, 0, 0, 0, 0)
		end
	end

	Citizen.Wait(1000)

	TriggerEvent('fuel:startFuelUpTick2', pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		if pumpObject then
			...
			DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Strings.CancelFuelingPump .. extraString)
			DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 0.5, Round(currentFuel, 1) .. "%")
		
		end

		--if not IsEntityPlayingAnim(SpawnedPed, "timetable@gardener@filling_can", "gar_ig_5_filling_can", 3) then
			--TaskPlayAnim(SpawnedPed, "timetable@gardener@filling_can", "gar_ig_5_filling_can", 2.0, 8.0, -1, 50, 0, 0, 0, 0)
		--end
	...
	RemoveAnimDict("timetable@gardener@filling_can")
	TaskPedSlideToCoord(SpawnedPed, stringCoords.x, stringCoords.y, stringCoords.z, 0, 500)
	Citizen.Wait(2000)
	TaskTurnPedToFaceEntity(SpawnedPed, vehicle, 1000)
	ClearPedTasks(SpawnedPed)
	Citizen.Wait(5000)
	...
	while true do
		...
		if not isFueling and ((isNearPump and GetEntityHealth(isNearPump) > 0) or (GetSelectedPedWeapon(ped) == 883325847 and not isNearPump)) then
			if IsPedInAnyVehicle(ped) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped), -1) == ped then
				...
				if engine then
					DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Strings.ExitVehicle)
			
				elseif GetVehicleFuelLevel(vehicle) < 95 then

					if currentCash > 0 then
						DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Strings.EToRefuel3)

						if IsControlJustReleased(0, 38) then
							
							isFueling = true

							TriggerEvent('fuel:refuelInCar', isNearPump, ped, vehicle)
